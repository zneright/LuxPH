...
impl InvoicePaymentContract {
    ...
    pub fn set_merchant_lock_duration(env: Env, lock_duration_seconds: i64) {
        let merchant = env.invoker();
        merchant.require_auth();
        env.storage().instance().set(&DataKey::MerchantLockDuration(merchant), &lock_duration_seconds);
    }
    ...
    pub fn pay_invoice(
        ...
        token_client.transfer_from(&customer, &contract_address, &amount);

        let contingency_amount = amount * contingency_percentage / 100;
        let merchant_share = amount - contingency_amount;

        if merchant_share > 0 {
            token_client.transfer(&merchant, &merchant_share);
        }
    ...
    pub fn withdraw_contingency(env: Env, token: Address) {
        let merchant = env.invoker();
        merchant.require_auth();
        ...
        let token_client = token::Client::new(&env, &token);
        token_client.transfer(&merchant, &payout_amount);
    ...
    pub fn get_lock_duration(env: Env, merchant: Address) -> i64 {
        env.storage()
            .instance()

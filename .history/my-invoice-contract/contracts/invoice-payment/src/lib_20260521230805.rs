...
impl Default for InvoiceRecord {
    fn default() -> Self {
        Self {
            merchant: Address::Contract(ContractAddress::from([0u8; 32])),
            customer_name: String::new(),
            amount: 0,
            token: Address::Contract(ContractAddress::from([0u8; 32])),
            memo: String::new(),
            created_at: 0,
        }
    }
...
impl InvoicePaymentContract {
    ...
    pub fn pay_invoice(
        ...
        env.events().publish(
            ...
            (customer, merchant.clone(), amount, contingency_amount, unlock_timestamp),
        );
    }

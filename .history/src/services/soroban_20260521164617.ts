...
export async function buildSorobanContractInvocationTransaction(options: {
  ...
  const horizonServer = new Horizon(options.horizonUrl);
...
export async function prepareSorobanTransaction(transaction: any, sorobanRpcUrl: string, networkPassphrase: string) {
...
export async function submitSorobanTransaction(signedTransaction: any, sorobanRpcUrl: string) {
  const sorobanServer = new SorobanServer(sorobanRpcUrl);
  return sorobanServer.sendTransaction(signedTransaction);
}

...
export default function CreateInvoice() {
  ...
  // Explicit arguments prevent stale state bugs when saved from inside the listener closure
  ...
  const processInstantVaultDeduction = async (paidAmount: string) => {
    ...
    try {
      ...
      // Successfully vaulted - Log Telemetry
  ...
  const handleStartListening = async () => {
    ...
    const now = Date.now();
    paymentStartTimeRef.current = now; // Store instantly using a ref to prevent 0.00 bug

    // Pass the EXACT CURRENT state into the listener to prevent stale state on subsequent invoices
  ...
  // We enforce parameters so the closure uses the locked-in active values, not older state

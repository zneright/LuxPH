...
export default function CashOut() {
  ...
  const handleCashOut = async () => {
    ...
    try {
      ...
      // FIX: Cryptographically validate the destination and fallback intelligently
      let anchorDestination = systemConfig.pdaxAnchorAddress;

      // Check if pdaxAnchorAddress is missing or invalid
      if (!anchorDestination || !StrKey.isValidEd25519PublicKey(anchorDestination)) {
        ...
        if (!asset.isNative() && asset.issuer) {
          // If sending PHPC/USDC, send it back to the issuer (which acts as the gateway)
          anchorDestination = asset.issuer;
        } else {
          // If sending XLM, use the PHPC issuer as a safe, funded destination

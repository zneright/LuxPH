    ...
    const executePayment = async (scannedDest?: string, scannedAmount?: string, scannedToken?: string) => {
        ...
        try {
            ...
            if (useContractPayment) {
                ...
                if (!effectiveContractId || !effectiveContractId.trim()) return alert("Soroban contract ID is not configured.\n\nTo deploy the payment contract:\n1. Run 'soroban contract build' in my-invoice-contract/contracts/invoice-payment/\n2. Run 'soroban contract deploy' with your contract WASM\n3. Copy the deployed contract ID to Platform Config\n\nContact your system administrator for deployment help.");
                if (!effectiveContractId.startsWith("C")) return alert("Invalid contract ID. Soroban contract IDs must start with 'C'.");
                if (!contractFunctionName || !contractFunctionName.trim()) return alert("Please enter the Soroban contract function name.");
                if (!networkConfig?.sorobanRpcUrl) return alert("Soroban RPC URL is not configured in system settings.");

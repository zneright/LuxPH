...
export default function CashOut() {
  ...
  const handleCashOut = async () => {
    ...
    try {
      ...
      // Extract signed XDR from Freighter response - handle both string and object responses
      let signedXdrString = "";
      if (typeof signResponse === "string") {
        ...
        // Fallback: try to get first string value from response object
        const values = Object.values(signResponse).filter(v => typeof v === "string");
        signedXdrString = values.length > 0 ? values[0] as string : "";
      }

      if (!signedXdrString) {
        throw new Error("Failed to extract signed transaction from wallet. Invalid response format.");
      }

      // Verify the signed transaction can be parsed before submitting
      try {
        TransactionBuilder.fromXDR(signedXdrString, sysConfig.networkPassphrase);
      } catch (parseErr) {
        throw new Error("Signed transaction XDR is invalid. Please try signing again.");
      }

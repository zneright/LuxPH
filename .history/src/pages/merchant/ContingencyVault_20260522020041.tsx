...
interface VaultConfig {
    ...
    networkUrl: string;
    targetAsset: string;
    backendSecretKey?: string | null; // Added for the Cloud Function
}
...
export default function ContingencyVault() {
    ...
    const [secretKey, setSecretKey] = useState<string>(localStorage.getItem("lux_vault_sk") || "");
    ...
    // Auto-unlock if a key is found in local storage
    useEffect(() => {
        if (secretKey && !isSessionUnlocked && !isAuthenticating && !keyError) {
            handleUnlockSession();
        }
    ...
    const handleUnlockSession = async () => {
        ...
        try {
            ...
            localStorage.setItem("lux_vault_sk", secretKey);
            setIsSessionUnlocked(true);
            addLog("Dashboard unlocked. Linked to Cloud Engine.", "success");

        } catch (err) {
            setKeyError("Invalid Secret Key or Network Error.");
            localStorage.removeItem("lux_vault_sk");
            setSecretKey("");
        } finally {
    ...
    const saveSettings = async (newConfig: VaultConfig) => {
        ...
        if (auth.currentUser) {
            ...
            // THE CLOUD FIX: We inject the secretKey into Firestore ONLY if the engine is enabled.
            // If disabled, we pass null to wipe it from the cloud for security.
            const cloudPayload = {
                ...newConfig,
                backendSecretKey: newConfig.isEnabled ? secretKey : null
            };

            await setDoc(userRef, { vaultConfig: cloudPayload }, { merge: true }).catch(err => {
                console.error("Failed to sync settings to Cloud", err);
            });

            if (newConfig.isEnabled) {
                addLog("Cloud Engine ACTIVATED. Listening 24/7.", "success");
            } else {
                addLog("Cloud Engine HALTED. Automation paused.", "warn");
            }
        }
    };
    ...
    return (
      ...
      `}</style>
            ...
            <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: "#9ca3af", fontSize: 14, marginTop: 6 }}>24/7 Automated ledger-locked savings engine powered by Cloud Functions.</p>
            </div>
            ...
            {!isSessionUnlocked ? (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="vault-card" style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
                    ...
                    <p style={{ color: "#9ca3af", fontSize: 13, marginBottom: 24, lineHeight: 1.6 }}>
                        To access your vault and manage automation rules, your secret key is required.
                        <br /><span style={{ color: "#f59e0b" }}>Your key is securely synced to Firebase Cloud Functions for 24/7 execution.</span>
                    </p>
                    ...
                    >
                        {isAuthenticating ? "Synchronizing Ledger Data..." : "Authenticate & Open Vault"}
                    </button>
                </motion.div>
            ) : (
                <div style={{ display: "grid", gap: "24px" }}>
                    ...
                    <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.1)", marginBottom: 16 }}>
                        ...
                        <button className={`tab-btn ${activeTab === 'settings' ? 'active' : ''}`} onClick={() => setActiveTab('settings')}>Cloud Engine Settings</button>
                    </div>
                        ...
                        {activeTab === 'dashboard' ? (
                            <motion.div key="dashboard" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="vault-card">
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20, marginBottom: 32 }}>
                                    ...
                                    <div className="stat-box">
                                        <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#9ca3af", letterSpacing: ".06em", marginBottom: 8, textTransform: "uppercase" }}>Cloud Engine Status</div>
                                        <div style={{ fontSize: 18, fontWeight: 800, color: config.isEnabled ? "#10b981" : "#ef4444", marginTop: 8 }}>
                                            {config.isEnabled ? "☁️ ACTIVE (24/7 Sync)" : "HALTED"}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ marginBottom: 16 }}>
                                    <button onClick={() => syncHorizonData(Keypair.fromSecret(secretKey))} style={{ background: "transparent", color: "#a78bfa", border: "1px solid rgba(167, 139, 250, 0.3)", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: "bold" }}>
                                        🔄 Refresh Balances
                                    </button>
                                </div>
                                ...
                                <h3 style={{ fontSize: 18, fontWeight: 800, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 12, marginTop: 40, marginBottom: 20 }}>Dashboard Telemetry</h3>
                            ...
                            <motion.div key="settings" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="vault-card">
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
                                    ...
                                    <div>
                                        <div style={{ marginBottom: 24 }}>
                                            <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }}>
                                                <div>
                                                    <div style={{ fontSize: 16, fontWeight: 800 }}>Master Cloud Toggle</div>
                                                    <div style={{ fontSize: 12, color: "#9ca3af" }}>Firebase syncs every 2 mins to auto-vault.</div>
                                                </div>
                                                <div style={{ width: 50, height: 26, background: config.isEnabled ? "#10b981" : "rgba(255,255,255,0.1)", borderRadius: 20, position: "relative", transition: "0.3s" }} onClick={() => saveSettings({ ...config, isEnabled: !config.isEnabled })}>
                                        ...
                                        <div style={{ marginBottom: 24 }}>
                                            ...
                                            />
                                            <div style={{ textAlign: "right", fontSize: 14, fontWeight: 800, color: "#a7f3d0", marginTop: 8 }}>{config.deductionPercentage}% per transaction</div>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ padding: 16, background: "rgba(16, 185, 129, 0.1)", border: "1px solid rgba(16, 185, 129, 0.2)", borderRadius: 12 }}>
                                            <div style={{ fontSize: 13, fontWeight: 800, color: "#a7f3d0", marginBottom: 6 }}>Cloud Architecture Note</div>
                                            <div style={{ fontSize: 12, color: "#d1d5db", lineHeight: 1.5 }}>
                                                When you enable the master toggle, your encrypted ruleset is deployed to Firebase. The cloud engine will monitor your wallet and process deductions 24/7, even when you are offline.
                                            </div>
                                        </div>
                                    </div>

...
export default function Settings() {
  ...
  const [walletNetwork, setWalletNetwork] = useState<WalletNetwork>(WalletNetwork.TESTNET);
        ...
        try {
          ...
          if (configSnap.exists()) {
            ...
            if (configData.stellarNetwork === "Mainnet (Public)") {
              setWalletNetwork(WalletNetwork.PUBLIC);
            } else {
              setWalletNetwork(WalletNetwork.TESTNET);
            }
          }
        } catch (err) {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      const wallet = new Wallet(walletNetwork);
      const connectedWallet = await wallet.getWalletInstance();

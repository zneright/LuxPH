...
// NEW: Accept isSubscribed as a prop from Settings
interface ContingencyVaultProps {
    isSubscribed: boolean;
}

export default function ContingencyVault({ isSubscribed }: ContingencyVaultProps) {
    ...
    const [isAuthenticating, setIsAuthenticating] = useState<boolean>(false); // Tracks pre-load state
    ...
    const syncHorizonData = async (kp: Keypair) => {
        ...
        try {
            ...
            throw error; // Throw to catch it in the auth function
        } finally {
            setIsSyncing(false);
        }
    };

    // THE FIX: Fully await the data sync BEFORE unlocking the UI
    const handleUnlockSession = async () => {
        try {
            ...
            // Now that data is perfectly processed, unlock the dashboard
    ...
    return (
      ...
      `}</style>
            ...
            {/* THE FIX: Render Pro Lock Screen if not subscribed */}
            {!isSubscribed ? (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="vault-card" style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
                    <div style={{ fontSize: 50, marginBottom: 16, filter: "drop-shadow(0 0 20px rgba(167, 139, 250, 0.4))" }}>⭐</div>
                    <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8, color: "#a78bfa" }}>Pro Exclusive Feature</h2>
                    <p style={{ color: "#9ca3af", fontSize: 14, marginBottom: 32, lineHeight: 1.6 }}>
                        The Automated Contingency Vault is a premium feature. Upgrade your account to automatically intercept incoming payments and create immutable, time-locked savings allocations directly on the Stellar ledger.
                    </p>
                    <button style={{ width: "100%", background: "linear-gradient(135deg,#7c3aed,#4f46e5)", color: "#fff", border: "none", borderRadius: 14, padding: "16px", fontWeight: 800, fontSize: 16, cursor: "not-allowed", opacity: 0.8, boxShadow: "0 8px 25px -6px rgba(124, 58, 237, 0.5)" }}>

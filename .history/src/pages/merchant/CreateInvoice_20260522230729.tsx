...
export default function CreateInvoice() {
  ...
  const processInstantVaultDeduction = async (paidAmount: string) => {
    ...
    try {
      ...
      const server = new Horizon.Server(vaultConfig.horizonUrl); // Use system config URL

      // --- CRITICAL FIX: Fetch account freshly right before signing ---
      ...
      // Log the exact Stellar error code to your console
      console.error("Instant vault lock failed:", error.response?.data?.extras?.result_codes || error);
    }
  };

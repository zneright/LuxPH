  ...
  const handleCashOut = async () => {
    ...
    try {
      ...
      if (useContractCashout) {
        ...
        // Remove the .split(",") logic entirely and replace it with this:

        const amountInStroops = Math.floor(parseFloat(tokenAmount) * 10000000);
        const tokenAddress = selectedToken === "PHPC" ? sysConfig.phpcIssuer : sysConfig.usdcIssuer;

        const parsedArgs = [
          merchantAddress,                          // 1. Merchant Address
          tokenAddress,                             // 2. Token Address
          amountInStroops,                          // 3. Amount in stroops (integers)
          sysConfig.anchorAddress || FALLBACK_ANCHOR // 4. PDAX Anchor Address
        ];

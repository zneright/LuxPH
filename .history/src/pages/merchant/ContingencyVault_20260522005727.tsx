...
export default function ContingencyVault() {
    ...
    const syncHorizonData = async (kp: Keypair) => {
        ...
        try {
            ...
            const parsedLocks: LockedFund[] = claimables.records.map((record) => {
                ...
                // Extract time predicate logic dynamically
    ...
    const executeVaultLock = async (kp: Keypair, amountToLock: string) => {
        try {
            ...
            // Build Claimable Balance Predicate: Cannot be claimed BEFORE unlockTimeISO
            const strictTimePredicate = ClaimPredicate.not(
                ClaimPredicate.beforeAbsoluteTime(unlockTimeISO)
            );
    ...
    const handleClaimFunds = async (lockId: string) => {
        ...
        try {
            ...
            // Horizon will automatically reject if the time predicate hasn't been met.

...
export default function CreateInvoice() {
  ...
  const handleStartListening = async () => {
    ...
    if (useContractInvoice) {
      ...
      if (!effectiveContractId || !effectiveContractId.trim()) {
        return alert("Soroban contract ID is not configured.\n\nTo deploy the invoice contract:\n1. Run 'soroban contract build' in my-invoice-contract/contracts/invoice-payment/\n2. Run 'soroban contract deploy' with your contract WASM\n3. Copy the deployed contract ID to Platform Config\n\nContact your system administrator for deployment help.");
      }
      if (!effectiveContractId.startsWith("C")) {
        return alert("Invalid contract ID. Soroban contract IDs must start with 'C'.");
      }
      if (!contractFunctionName || !contractFunctionName.trim()) {
        return alert("Please provide the contract function name.");
      }
      if (!networkConfig?.sorobanRpcUrl) {
        return alert("Soroban RPC URL is not configured in system settings.");
      }
      ...
      try {
        ...
        if (errorMsg.includes("non-existing value") || errorMsg.includes("contract instance")) {
          return alert(`The contract (${effectiveContractId}) is not deployed on the current network.\n\nTo fix this:\n1. Build: soroban contract build (in my-invoice-contract/contracts/invoice-payment/)\n2. Deploy: soroban contract deploy --wasm <wasm-file>\n3. Update Platform Config with the new contract ID\n\nIf the contract ID in Platform Config is incorrect, update it in the Admin Settings.`);
        }
        if (errorMsg.includes("insufficient auth")) {
          return alert("Failed to authorize the contract invocation. Please ensure your wallet is properly connected and has sufficient XLM for transaction fees.");
        }
        if (errorMsg.includes("simulation")) {
          return alert(`Contract simulation failed.\n\nThis could mean:\n- The contract function signature doesn't match your arguments\n- The contract requires different argument types (e.g., expected Address, got String)\n- The contract hasn't been deployed on this network yet\n\nPlease verify the contract is correctly deployed and the function arguments match.`);
        }
        return alert(`Soroban invoice registration failed: ${errorMsg}`);
      }
    }

...
export default function ContingencyVault() {
    ...
    const executeVaultLock = async (kp: Keypair, amountToLock: string) => {
        try {
            ...
            // Build Claimable Balance Predicate: Cannot be claimed BEFORE unlock time
            const strictTimePredicate = ClaimPredicate.not(
                ClaimPredicate.beforeAbsoluteTime(unlockUnixSeconds)
            );

            const op = Operation.createClaimableBalance({
                asset: asset,
                amount: amountToLock,
                // Use the proper Claimant class constructor
                claimants: [
    ...
    const saveSettings = async (newConfig: VaultConfig) => {
        ...
        if (auth.currentUser) {
            ...
            await setDoc(userRef, { vaultConfig: newConfig }, { merge: true });
        }

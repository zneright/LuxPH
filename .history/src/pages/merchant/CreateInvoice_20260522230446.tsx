...
export default function CreateInvoice() {
  ...
  // --- NEW: INSTANT DEDUCTION LOGIC ---
  const processInstantVaultDeduction = async (paidAmount: string) => {
    if (!vaultConfig || !vaultSecretKey) return;
    ...
    try {
      ...
      const networkPassphrase = vaultConfig.networkUrl.includes("testnet") ? Networks.TESTNET : Networks.PUBLIC;
      ...
      if (auth.currentUser) {
        const telemetryRef = collection(db, "telemetryLogs");
        await setDoc(doc(telemetryRef), {
          merchantId: auth.currentUser.uid,
          message: `POS Instant Intercept! Vaulted ${deduction.toFixed(7)} ${vaultConfig.targetAsset}`,
          timestamp: new Date().toISOString(),
          type: "success"
        });
      }
    } catch (error) {
      console.error("Instant vault lock failed:", error);
    }
  };
  // ------------------------------------

...
export default function ContingencyVault() {
    ...
    // Default Settings (Overwritten by Firestore if they exist)
    ...
    const processedTxs = useRef<Set<string>>(new Set()); // Replay attack prevention

    // --- 1. Initialization & Config Load (From Firestore) ---
    useEffect(() => {
        const initVault = () => {
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    ...
                    if (mDoc.exists()) {
                        ...
                        // Only load the configuration settings. Funds are NOT pulled from Firestore.
                        if (data.vaultConfig) {
    ...
    // --- 2. Horizon Sync Service (Funds directly from Blockchain) ---
    const syncHorizonData = async (kp: Keypair) => {
        ...
        try {
            ...
            // Fetch Claimable Balances (Locked Funds)
            ...
            const parsedLocks: LockedFund[] = claimables.records.map((record) => {
                ...
                // Extract time predicate logic dynamically (Unix Timestamp)
                ...
                if (predicate && predicate.not && predicate.not.abs_before) {
                    // Horizon returns an ISO date string (e.g., "2026-06-21T01:19:35Z")
                    // We parse it directly back into a standard JavaScript timestamp
    ...
    // --- 3. Secure Session & Engine Activation ---
    ...
    // --- 4. Payment Listener & Auto-Vaulting Engine ---
    ...
    // --- 5. Immutable On-Chain Lock Creation ---
    const executeVaultLock = async (kp: Keypair, amountToLock: string) => {
        try {
            ...
            // Calculate absolute unlock time limit
            const unlockDate = new Date();
            unlockDate.setDate(unlockDate.getDate() + config.lockDurationDays);

            // Convert standard JS date to a Unix Timestamp in seconds (required by Stellar)
            const unlockUnixSeconds = Math.floor(unlockDate.getTime() / 1000).toString();

            // JS/TS SDK syntax for Claimants
    ...
    // --- 6. Early Redemption Prevention / Claiming ---
    ...
    // --- 7. Settings Persistence (Saves ONLY settings to Firestore) ---
    const saveSettings = async (newConfig: VaultConfig) => {
        setConfig(newConfig);

        // Save ONLY the Vault configuration to Firestore
        if (auth.currentUser) {
        ...
        // Restart listener if settings changed and session is active
    ...
    return (
      ...
      `}</style>
            ...
            {/* Header section matching platform aesthetics */}
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'dashboard' ? (
                            <motion.div key="dashboard" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="vault-card">
                                ...
                                {/* Locked Funds Feed */}
                                <h3 style={{ fontSize: 18, fontWeight: 800, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 12, marginBottom: 20 }}>Encrypted Allocations</h3>

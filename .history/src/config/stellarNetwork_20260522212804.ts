...
export function buildStellarNetworkConfig(config?: Partial<StellarNetworkConfig>): StellarNetworkConfig {
    ...
    return {
        ...baseConfig,
        horizonUrl: config?.horizonUrl || baseConfig.horizonUrl,
        sorobanRpcUrl: config?.sorobanRpcUrl || baseConfig.sorobanRpcUrl,
        explorerTxUrl: config?.explorerTxUrl || baseConfig.explorerTxUrl,
    };
}

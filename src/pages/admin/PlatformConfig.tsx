...
interface GlobalConfig {
  ...
  usdcIssuerAddress: string; // Dynamic field alignment
  pdaxAnchorUrl: string;
  stellarNetwork: string; // Toggles system runtime network
}
...
const DEFAULT_CONFIG: GlobalConfig = {
  ...
  phpcIssuerAddress: "GBSTRH776KCSX6NRE4LHYOM3E5O6F4PC01MAINNETISSUER", // Replace with fallback
  usdcIssuerAddress: "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5", // Replace with fallback
  pdaxAnchorUrl: "https://anchor.pdax.ph",
  stellarNetwork: "Testnet (Futurenet)" // Defaults to testnet deployment state
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} style={{ padding: "4px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
        ...
        <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.05)", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,.05)", fontSize: 13, fontWeight: 700, color: "#f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ padding: 20, opacity: isNetworkConfigUnlocked ? 1 : 0.65, transition: "opacity 0.2s" }}>
            ...
            <div>
              <label style={{ display: "block", fontSize: 10, fontFamily: "'DM Mono',monospace", color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>
                PDAX Anchor Liquidity Gateway URL
              </label>
              <input
                type="text"
                disabled={!isNetworkConfigUnlocked}

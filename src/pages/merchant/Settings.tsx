...
interface MerchantData {
  ...
  isSubscribed?: boolean; // Added this to match the UI logic
  preferences?: {
    currency: string;
...
export default function Settings() {
  ...
  // --- NEW: State for the Monthly Usage Card ---
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        // 2. Fetch Invoices to Calculate Monthly Usage
        try {
          ...
          snapshot.forEach((docSnap) => {
            ...
            if (data.timestamp) {
              ...
              // Only sum up invoices from the current month and year
  ...
  return (
    <div>
          ...
          <div style={{ padding: 20 }}>
            {stellarAddress ? (
              ...
              <>
                ...
                <button onClick={connectWallet} disabled={isConnecting} style={{ background: "linear-gradient(135deg,#7c3aed,#4f46e5)", color: "#fff", border: "none", borderRadius: 7, padding: "10px 16px", fontSize: 13, cursor: isConnecting ? "not-allowed" : "pointer", fontFamily: "'Nunito',sans-serif", fontWeight: 700 }}>
      ...
      {/* RENDER THE INVOICE DASHBOARD HERE */}
      {stellarAddress && user && (
